...
Companion to [REPORT.md](REPORT.md) (the short-task suite). Same fairness
controls: **same model** (`claude-sonnet-4.6`) for both, fresh isolated
...
faster on the expression evaluator. Same model → both produced correct, idiomatic
designs; looplet's cartridge nudges more structured/verbose code, Copilot trends
more compact.
...
### Code design — same model, different defaults
...
because the model does the heavy lifting and both harnesses feed it competently.
looplet's structural edge remains **ownership**: same results, tighter test/fix
