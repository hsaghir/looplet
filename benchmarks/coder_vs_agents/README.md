...
A small, reproducible, **requested-model-family-matched** comparison of looplet's
...
Both paths requested `claude-sonnet-4.6`, reducing model-family differences.
Looplet used a local Copilot proxy while Copilot CLI used its own connection,
so serving infrastructure, hidden policy, caching, and sampling may still
differ. This is an end-to-end harness-and-serving-path comparison, not a pure
causal isolation of scaffolding or model intelligence.
...
## Headline findings (requested model family: `claude-sonnet-4.6`)
...
- An OpenAI-compatible endpoint for looplet's LLM. To match the requested model family,
...
- **End-to-end comparison, not a controlled model experiment** - both paths
  request the same model family through different serving connections.
