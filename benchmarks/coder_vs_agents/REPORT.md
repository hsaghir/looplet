...
**TL;DR** - With both paths requesting `claude-sonnet-4.6`, the looplet
...
> This is an end-to-end harness-and-serving-path comparison. Looplet used a
> proxy and Copilot used its own connection. Matching the requested model family
> reduces one confound but does not make serving policy, caching, or sampling
> identical, so differences cannot be attributed to scaffolding alone.
...
On these well-scoped, verifiable tasks, both paths were reliable. Neither made a
logic error that survived to the verifier. The sample supports observed parity
on this suite, not a general claim about either harness or serving path.

### 2. Speed - **looplet faster (won 7/9)**

looplet averaged 18.3 s vs Copilot's 23.1 s. One observed contributor is
**startup overhead**: Copilot boots MCP servers (`github-mcp-server`, `devtools`,
...
Both paths produced similar algorithms with different presentation defaults:
...
1. **Requested model family, different serving paths** - both requested
   `claude-sonnet-4.6`, but the proxy and Copilot connection may differ in
   hidden policy, caching, and sampling.
