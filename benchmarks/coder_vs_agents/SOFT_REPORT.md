...
(both requested `claude-sonnet-4.6` through separate serving paths, with a fresh
workspace). Quality was then
...
and one plausible contributor is the **system prompt**: looplet's example
cartridge is a **coder**, tuned for terse, action-oriented engineering;
Copilot is a broader generalist assistant. Separate serving paths and
single-generation samples prevent causal attribution.
...
Both effectively **one-shot** each answer, so loop iteration did not explain the
observed gap. Prompt/persona and serving-path differences both remain possible
contributors.
...
- Matched requested model family, both one-shot: the **system prompt / harness
  persona** is a plausible contributor. looplet's shipped example is
  `coder.cartridge`, whose
...
produced noticeably more depth in this sample. The fix on the looplet
...
**The rerun's quality gap closed ~62%** (1.62 → 0.62 points),
looplet improved on **5 of 6** tasks, and it went from **0 task wins to 1 win
and 2 ties**. The biggest lifts were the pure-explanation tasks (TLS, CAP, tail
...
**Takeaway:** after ~4 lines of prompt changed, this single rerun closed most of
the non-coding gap without an observed coding regression. That supports a
follow-up with repeated samples; it does not prove a general causal effect. For the last mile
