...
def _build_briefing(state: Any, session_log: SessionLog, context: Any) -> str:
    ...
    wrote_files = [
        s.tool_call.args.get("file_path", "?")
        for s in steps
    ...
    for s in reversed(steps):
        if (
            s.tool_call
            and s.tool_call.tool == "bash"
            and "pytest" in s.tool_call.args.get("command", "")
        ):
...
def eval_error_recovery(ctx: EvalContext) -> dict:
    ...
    """
    errors = [s for s in ctx.steps if getattr(s.tool_result, "error", None)]
...
def main() -> None:
    ...
    parser.add_argument(
        "task",
        nargs="?",
        default="Implement a fibonacci function in fibonacci.py with tests in test_fibonacci.py",
        help="What to build (default: fibonacci)",
    )
    parser.add_argument("--max-steps", type=int, default=15, help="Max agent steps (default: 15)")
