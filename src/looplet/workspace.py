...
def _render_hook_source(cls: type, warnings: list[str], strict: bool) -> str:
    ...
    Preference order:
      ...
      2. Module source available → dump full module (preserves imports).
      3. Class source only → dump source with a typing-import fallback.
    """
    cls_name = cls.__name__
    ...
    if module_name and module_name not in {"__main__", "builtins"}:
        try:
            ...
            # Fall through to source-dump strategy.
            pass

    # 2. Fall back to dumping the FULL module source (captures imports,
    ...
    # 3. Last resort: just the class source. Likely needs hand-editing on
