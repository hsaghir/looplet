...
Covers:
  ...
  4. agent_factory.cartridge loads + extends coder.cartridge correctly
...
def test_extends_cycle_detection(tmp_path: Path) -> None:
    ...
    with pytest.raises(CartridgeSerializationError, match="circular"):
        cartridge_to_preset(a)
...
def test_extends_missing_parent_raises(tmp_path: Path) -> None:
    ...
    with pytest.raises(CartridgeSerializationError, match="does not exist"):
...
def test_agent_factory_workspace_loads() -> None:
    ...
    ws = repo_root / "examples" / "agent_factory.cartridge"
    p = cartridge_to_preset(str(ws), runtime={"workspace": "."})
...
def test_validate_workspace_tool_missing_workspace_json(tmp_path: Path) -> None:
    ...
    assert r.data.get("missing") == "cartridge.json"
