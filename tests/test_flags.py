"""Tests for looplet.flags — _Flags and FLAGS singleton."""
...
class TestFlagDefaults:
    def test_singleton_exists(self):
        from looplet.flags import FLAGS, _Flags
...
class TestFlagEnvOverrides:
    """Test with LOOPLET_* prefix (canonical)."""

    def test_concurrent_dispatch_env_true(self, monkeypatch):
        monkeypatch.setenv("LOOPLET_CONCURRENT_DISPATCH", "1")
...
class TestIntFlags:
    def test_sub_agent_max_steps_env(self, monkeypatch):
        monkeypatch.setenv("LOOPLET_SUB_AGENT_MAX_STEPS", "10")
        from looplet.flags import _Flags

        assert _Flags().sub_agent_max_steps == 10

    def test_sub_agent_max_spawns_env(self, monkeypatch):
        monkeypatch.setenv("LOOPLET_SUB_AGENT_MAX_SPAWNS", "5")
        from looplet.flags import _Flags
...
class TestExports:
    ...
    def test_no_harness_prefix_in_flags(self):
        """Verify we use LOOPLET_ prefix for env vars (no legacy env var prefixes)."""
        import looplet.flags as fm
