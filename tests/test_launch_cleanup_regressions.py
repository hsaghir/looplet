...
from scripts import check_text_style
...
def test_conformance_failure_output_uses_ascii_separator(tmp_path, monkeypatch, capsys):
    ...
    def load(path, *, strict, runtime):
        assert strict is True
        assert callable(runtime["ask_handler"])
        if "broken" in path:
...
def test_describe_closes_loaded_preset(tmp_path, monkeypatch):
    from looplet import cartridge
